# ZCode Ops

Current status (2026-09-08): **Existing desktop task monitoring, model selection, and messaging are available through ZCode Remote Control.** `zcode_remote_tasks` lists the live desktop task inventory, `zcode_remote_read` reads a conversation snapshot, `zcode_remote_models` reads its model options, `zcode_remote_set_model` changes an idle task's model, and `zcode_remote_send` sends a user-authorized prompt. Legacy ACP creation, loading and prompting remain suspended because the dependency hardcodes native `mode: "yolo"` and writes the desktop index directly.

The remote bridge uses the link generated by ZCode's lower-left Mobile Remote Control entry. Starting Codex never prompts for it. On the first requested ZCode operation, the saved link is used; if connection fails, Codex asks the user for the current link and calls `zcode_config_set` before one explicit retry. It is stored as `zcode-ops/config.json` under `CODEX_HOME`, outside the installed plugin, so reinstalling or upgrading does not overwrite it. The link is bearer authorization for the ZCode window; it is never logged, committed, or returned by status tools. The adapter serializes connections and never retries an uncertain send.

The installed plugin exposes `zcode_config_status`, `zcode_config_set`, and `zcode_config_clear`; there is no custom desktop dialog. The checked-in [config.json.example](config.json.example) and [config.schema.json](config.schema.json) document the format. A repository-root `config.json` is migration-only and ignored; the live copy belongs under `CODEX_HOME` so plugin packaging cannot copy the credential.

`zcode_remote_read` uses ZCode's `resumeModelPolicy: "ui-resolved-only"` to avoid reapplying stale model hints from the desktop index. This fixes reading after a model recovery without another model switch or prompt. History remains partial when ZCode limits the snapshot.

`zcode_remote_wait` waits for one task's native status to change, returning its status and a `cursor` for `afterCursor` on the next call. `timeoutMs` is 0–30000 (default 30000); network request latency can extend the wait window. A zero timeout takes one snapshot. Completed, failed, idle, waiting, unknown, or missing tasks return immediately. Streaming text and `updatedAt` changes alone do not wake the wait. Timeout never means interruption. Each poll releases the remote connection before sleeping, allowing other operations between polls; no background monitor or startup connection is created.

`zcode_remote_cancel` requests ZCode's native `stopGeneration` for the specified running task. It checks status again after attaching and returns `not_running` without sending if the turn has ended. `cancel_requested` means acknowledgment only; use wait/read to confirm how the turn ended. The ZCode stop endpoint targets the current turn by task ID, so avoid sending a new turn concurrently with cancellation. It never kills ZCode or retries an uncertain stop.

The remote surface is deliberately agent-neutral: task selection, state normalization, snapshot reading, and prompt delivery are separate from the worker model. Future adapters can use the same MCP contract for other desktop intelligent agents without exposing arbitrary installation or model-routing tools.

The remaining inventory/event tools are diagnostic. The legacy name `zcode_desktop_sessions_status` returns the native CLI session inventory, not the desktop's visible task list or authoritative running status. A native session or a row in the desktop index does not establish desktop creation. A verified native desktop route is required before execution can be restored. Reinstall and start a new Codex task to load this version; already-running older gateways are not hot-patched.

The real desktop tools require a fresh Codex task after reinstall:

```powershell
node scripts/remote-call.mjs zcode_config_status '{}'
node scripts/remote-call.mjs zcode_remote_tasks '{}'
node scripts/remote-call.mjs zcode_remote_read '{"taskId":"sess_..."}'
node scripts/remote-call.mjs zcode_remote_wait '{"taskId":"sess_...","timeoutMs":30000}'
node scripts/remote-call.mjs zcode_remote_cancel '{"taskId":"sess_..."}'
node scripts/remote-call.mjs zcode_remote_models '{"taskId":"sess_..."}'
node scripts/remote-call.mjs zcode_remote_set_model '{"taskId":"sess_...","model":"deepseek-v4-flash-vision-exp"}'
node scripts/remote-call.mjs zcode_remote_send '{"taskId":"sess_...","prompt":"请回传当前工作进展"}'
```

```powershell
npm ci --ignore-scripts --registry=https://registry.npmjs.org
npm run smoke
```

The configured ZCode CLI is `D:\Programs\ZCode\resources\glm\zcode.cjs`. If it moves, change `mcacp.json` before querying the native inventory.

Both execution and inventory use `mcacp.json`'s agent environment. On this Windows installation, child-only `HOME=E:\Programming\IDE` routes ACP credentials and task-index access to the desktop's `.zcode/v2`; `ZCODE_DATA_BASE_DIR` routes native shared credentials to the same location. `USERPROFILE` and the native CLI database remain unchanged at `C:\Users\WQ\.zcode\cli\db\db.sqlite`, which already contains the desktop sessions. Do not relocate that database merely to match the desktop index. No credentials are copied or logged, and no system environment is changed.

The earlier claim that new sessions preserve `operator` permissions was incorrect: the underlying ACP sets `yolo` during native creation. No approval tool is exposed. The subsequent phases and correction record remain in [the plan](../../plans/2026-09-05-zcode-ops-integration-plan.md).

`zcode_desktop_sessions_status` starts ZCode's local ACP bridge and calls its native `session/list`. It returns only id, workspace, title, and update time—never message text. `recently_active` means a recent native update time, not that the task is running. The native list does not expose archived or compacting state. Correlate with Codex task state by workspace only.

Historical headless protocol behavior (not currently available for new prompts): `zcode_prompt_start` added a unique completion receipt. State is in-memory for the gateway process. `zcode_task_poll` consumes events; `zcode_bridge_session_status` reads the last observed state without consuming events or starting another ZCode backend. All event readers (`zcode_prompt`, `zcode_prompt_events`, `zcode_events`, `zcode_task_poll`) update the same state. They consume the same queue, so choose one reader per session.

`zcode_task_poll` returns `{ sessionId, status, terminal, receivedAt, event, ... }`. `event` is the unmodified MCP result. The bridge reads only MCACP protocol fields and the assistant's reply suffix; it does not depend on model identity.

| Evidence | State | Terminal |
| --- | --- | --- |
| Prompt accepted | `submitted` | false |
| Assistant text arrives | `running` | false |
| `permission_request` event | `waiting_permission` | false |
| Cancel notification sent | `cancel_requested` | false |
| `complete`, stopReason `cancelled` | `cancelled` | true |
| `error` event | `failed` | true |
| `complete` event | `turn_completed` | true |
| `complete`, `end_turn`, assistant reply ends with receipt | `completed_receipt` | true |

`terminal` means the turn ended, not that the task passed acceptance. A receipt is the agent's completion claim. Empty polling and stale activity do not prove interruption. Read/transport errors are returned separately (`readError` or JSON-RPC error); ordinary text containing `turn.failed` is never a protocol failure. Bridge session ids and native desktop ids may differ, so stale activity uses observed bridge timestamps rather than joining those ids.

Verification: `npm run smoke` includes protocol, profile-routing, and suspended-entry regression checks. The historical `live-smoke.mjs` is not a desktop acceptance test and cannot run while execution is suspended. Do not remove the suspension merely to make that probe pass.

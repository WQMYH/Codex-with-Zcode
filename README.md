...
The remote bridge uses the link generated by ZCode's lower-left Mobile Remote Control entry. Each plugin gateway startup opens a native Windows input window for the current link. It is stored as `zcode-ops/config.json` under `CODEX_HOME`, outside the installed plugin, so reinstalling or upgrading does not overwrite it. The link is bearer authorization for the ZCode window; it is never logged, committed, or returned by status tools. The adapter serializes connections and never retries an uncertain send.

The installed plugin exposes `zcode_config_status`, `zcode_config_prompt`, `zcode_config_set`, and `zcode_config_clear`. Prefer the prompt window when entering a link so the credential does not enter chat or tool history. Set `promptOnStartup` to `false` only if reusing the last saved link is intentional. The checked-in [config.json.example](config.json.example) and [config.schema.json](config.schema.json) document the format. A repository-root `config.json` is migration-only and ignored; the live copy belongs under `CODEX_HOME` so plugin packaging cannot copy the credential.
...
```powershell
node scripts/remote-call.mjs zcode_config_prompt '{}'
